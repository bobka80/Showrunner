...
**Preflight note (2026-07-15, Slice A):** Phase 1 inventory complete in [data-access-layer.md § Phase 1 preflight](data-access-layer.md#phase-1-preflight--inventory-2026-07-15). **GAS rule:** never put `*/` inside block comments; adapter methods must not reuse global `google.script.run` API names. v577 broke PA save (comment syntax error in `Dal_Repos.js`); fixed v578+.
