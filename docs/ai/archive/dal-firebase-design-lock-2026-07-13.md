...
|------|----------------------------------------|----------------------|
| **1 — authority** | Sheets = SoT between sessions; Firebase = live buffer until End | Firebase = **active workspace** for **five slices** (meta, PA, timeline, ledger, **ops**); Sheets = latest **published** durable record (may lag ≤ checkpoint interval). After End / idle close / between rooms: Sheets remains restore point. |
| **2 — Sheets writes** | No mid-session Sheets sync — one commit at close | **Periodic publish checkpoints** (~30m, dirty-only) allowed while room stays live. Final publish on Explicit End / N-day idle close. Order: **meta → PA → timeline → ledger → ops**. |

**Unchanged forever:** Rules 3–5 and 7 (failed syncs never silent; visible conflicts; cache isolation; concurrent domains allowed). No silent drop of failed publishes.
...
**Until R3c/R3d ship:** production meta identity and ops remain Sheets-primary; Hub already warm (R3b @ v745). This subsection is the doctrine text remaining slices must implement against.
