...
**Arrange UI mirror (2026-07-31 / v751–v752):** Ledger remains SoT, but live `assets/state` fixtures must carry camelCase truck fields after warm arrange (`saveTruckArrangementFirestore_` stamps from legs). Blank fixtures + live listener made save→reopen dump every case into staging. **v752:** Packing mode `explodeProjectAssets` deleted uids on qty clones → Arrange save dropped those cases; open Arrange now **collapses first**, hydrates by **replacing** PA from `getProjectAssets`, and save returns `current` fixtures for client replace. Explode now mints sibling uids.
