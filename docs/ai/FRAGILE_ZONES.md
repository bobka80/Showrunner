...
| Prep remote sync | Fast path: Firestore `projects/{id}/assets/_meta` — close only after **confirmed** server open then missing. Safety: Sheets poll — open + close. Fresh open: **~30s** IgnoreCloseUntil + two closed polls (or `committing` while latched). |
