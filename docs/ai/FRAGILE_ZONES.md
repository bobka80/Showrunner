...
**Arrange UI mirror (2026-07-31 / v751):** Ledger remains SoT, but live `assets/state` fixtures must carry camelCase truck fields after warm arrange (`saveTruckArrangementFirestore_` stamps from legs). Blank fixtures + live listener made save→reopen dump every case into staging. Client also mirrors save payload locally and hydrates via `getProjectAssets` when memory has no spatial data before opening Arrange.
