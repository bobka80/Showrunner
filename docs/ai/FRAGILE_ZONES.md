...
**Campaign Room R3 (2026-07-24):** Warm truck arrange writes `projects/{id}/logistics/state` (not Sheets). Prep open snapshots logistics alongside PA. `getProjectAssetsFirestore_` overlays Firebase legs when present. **END ROOM** commits logistics after PA/timeline (`dalCommitLogisticsFromFirestore_`). Tracker/Conflicts still read Sheets until R4/R5. Size WARN/MAX same as PA/timeline state. **GAS v744**.  
