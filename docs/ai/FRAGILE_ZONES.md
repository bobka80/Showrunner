...
**Campaign exit badge — batch vs live touch (R3e):** Whole-operation modals (Truck Arrange, Logistics Hub generate/pack/arrange) edit locally then **one batch commit** (logistics-first; PA collection rewrite only on structural uid change / new auto docs). Do **not** chat Firebase one-row GET+WRITE per case. Live multi-user floor edits stay small touch patches / absolute upserts of touched UIDs. Hub batch still open after Arrange batch ships.
