...
2. **Clear the prep latch on the first “closed” Sheets poll.** Prep UI latches open (`dalPrepLatched`) so flaky reads cannot flicker. Remote END PREP must use `allowClose: true` (Firestore `_meta` gone **or** confirmed Sheets poll). A close-only poll with no grace **wiped remote START PREP** when Sheets lagged Firebase (**v605 → v606**). After `_meta` delete, Sheets often still says `open`/`committing` — poll must **not re-latch** (`dalPrepIgnoreOpenUntil` + ignore `committing`/`opening` for new opens) or the remote banner stutter-loops and cannot leave prep (**v636→**). **Empty `_meta` while PA is open (no prep yet) must not be treated as END** — that set `IgnoreOpenUntil` and fought START PREP so START↔END buttons flipped on both screens (**v637→**). Meta-close: **one** server-missing snap after `dalMetaSeenOpen_` (delete fires once — dual-missing-snap left remote END stuck) + IgnoreCloseUntil grace (~8s). Poll: latched UI + Sheets `committing` = peer END in progress → allowClose after two ticks (**v639→**). **Never apply `_meta` fromCache** and **never reopen while `IgnoreOpenUntil`** — stale cache after DELETE reopened prep (cleared the grace), restarted live sync mid-commit → banner flash + items wander then settle (**v640→**).
...
| Prep remote sync | Fast path: Firestore `projects/{id}/assets/_meta` (**one** server-missing snap after SeenOpen). Safety: Sheets poll while PA open — **both** open and close. After a fresh open: **~8s grace** + **two consecutive closed polls** (or `committing` while latched) before `allowClose`. |
