...
**Campaign Room R3 (2026-07-24):** Warm truck arrange writes `projects/{id}/logistics/state` (not Sheets). Prep open snapshots logistics alongside PA. `getProjectAssetsFirestore_` overlays Firebase legs when present. **END ROOM** commits logistics after PA/timeline (`dalCommitLogisticsFromFirestore_`). Tracker/Conflicts warm hybrid = [active/warm-readers-offer-tracker-2026-08-05.md](active/warm-readers-offer-tracker-2026-08-05.md) **W2** (dalWarmReaderOneShotOverlay_ — cap 25, fail-open, **no** badge / listeners). Size WARN/MAX same as PA/timeline state. **GAS v744**.  
