...
> **CURRENT:** Prep **and** timeline live forks are **ON** (`DAL_LIVE_FORKS_PAUSED = false`). Ops: [topics/dal-live-forks-pause.md](topics/dal-live-forks-pause.md). Campaign Room warm/cold: [archive/project-campaign-room-2026-07-24.md](archive/project-campaign-room-2026-07-24.md).
...
**Idle / auto-close (Part B4 @ v703+):** Timeline idle **45m** / prep **75m**; T−5 SYNC **Session closing — tap to keep open**; last leave + idle call same `closeDalSession` commit path; station presence blocks prep idle eject; presence ping ~**45s**, server stale **150s**. **While Campaign Room warm:** Part B short idle never ejects (tick resets touch); last-leave soft-leaves. **R5:** room idle = `DAL_CAMPAIGN_IDLE_MS_` (default **48h**; set `168 * 60 * 60 * 1000` or `240 * …` for longer). Reset on five-slice WRITE or station dock (`dalTouchCampaignActivity_` / `touchDalCampaignActivity`) — **not** presence. Hourly `runDalCampaignIdleSweep` → `closeDalCampaignRoom`. Editor also checks ~15m while warm.
...
**Campaign Room R2 (2026-07-24):** Slice `_meta` carries `roomUid` (= `campaignRoomUid`). While room warm: last-leave / 45m·75m idle / orphan reclaim are **soft leave only** (no Sheets commit). Explicit **END ROOM** on the **project editor** (`#master-end-room-btn`) → `closeDalCampaignRoom` (publish both open domains, clear room). Module END PREP / END COLLAB hidden while warm. No post-commit auto-reopen after room End. Cold room keeps Part B short-session behavior. **R5 idle close live** via `DAL_CAMPAIGN_IDLE_MS_` + hourly sweep (warm rooms auto End after N-hour silence).

**Campaign Room R3 (2026-07-24):** Warm truck arrange writes `projects/{id}/logistics/state` (not Sheets). Prep open snapshots logistics alongside PA. `getProjectAssetsFirestore_` overlays Firebase legs when present. **END ROOM** commits logistics after PA/timeline (`dalCommitLogisticsFromFirestore_`). Tracker/Conflicts warm hybrid = [archive/warm-readers-offer-tracker-2026-08-05.md](archive/warm-readers-offer-tracker-2026-08-05.md) **W2** (dalWarmReaderOneShotOverlay_ — cap 25, fail-open, **no** badge / listeners). Size WARN/MAX same as PA/timeline state. **GAS v744**.  
**Arrange UI mirror (2026-07-31 / v751–v754):** Ledger remains SoT. Live fixtures must carry camelCase truck fields after warm arrange. **v754:** Auto-cases often missing from Firebase PA — Arrange now emits ledger legs for unmatched `paUid`s, upserts minimal auto PA docs, merges autos on save success, and delta state rebuild re-overlays logistics.

**PA live mirror wipe guard (2026-08-04):** Never publish a thin/empty `assets/state` when the PA collection still has rows. Delta/arrange rebuild state via collection re-list; `getProjectAssetsFirestore_` heals thin state from collection; client ignores thin `gas_*` mirrors and re-fetches.
...
**Campaign Room R4 (2026-08-04):** Keep-live ~30m publish (`DAL_CAMPAIGN_CHECKPOINT_MS_`, `runDalCampaignCheckpoint`). Dirty gate via `checkpointSigsJson`; ordered meta→PA→timeline→ledger→ops; **does not** delete Firebase forks or freeze room (END ROOM still tears down). Fail → pocket + cue; escalate if lag ≥ `DAL_CAMPAIGN_CHECKPOINT_ESCALATE_MS_` (~2h). Editor cue `#master-checkpoint-cue` — outlined button, label `{time} publish`, **one-click** force (confirm). Client timer ~60s while warm.

**Index identity save must preserve DAL columns (2026-08-10):** `saveProjectDataSheets_` used to blank-fill the whole `Projects_Index` row then `setValues` — that wiped `Dal_Campaign_*` and prep/timeline session columns. Force checkpoint / END ROOM meta publish and Save & Sync all hit this path. Fix: copy the existing Index row first, then overlay identity fields. Also refresh `#edit-proj-version` from checkpoint `indexLastUpdated` so Save & Sync does not false-collide after publish.
...
**Campaign Room R5 (2026-08-05):** `DAL_CAMPAIGN_IDLE_MS_` (default 48h). `dalTouchCampaignActivity_` on slice writes + `touchDalCampaignActivity` on station dock. Hourly `runDalCampaignIdleSweep` → `closeDalCampaignRoom`. Presence does not reset idle. Station project pick/dock calls `openOrJoinDalCampaignRoom_`. Tracker hybrid + Offer pulls → [archive/warm-readers-offer-tracker-2026-08-05.md](archive/warm-readers-offer-tracker-2026-08-05.md) (**no** Live badge). Room campaign archived 2026-08-05. Warm-readers campaign archived 2026-08-11 → successor [active/warm-live-completion-2026-08-11.md](active/warm-live-completion-2026-08-11.md).

**Warm-readers Exit (2026-08-11):** Offer stays frozen Pull+Undo; Tracker/Conflicts stay one-shot hybrid readers. Identity autosave still GAS→Firebase until Warm Live **L2**. Do not reopen Offer as live sync here.
...
**Warm Live L3 (2026-08-12):** While Campaign Room warm, START PREP / START COLLAB stay hidden (dalMayShowStartForkButton_). Opening PA / Timeline auto-attaches live forks (maybeAutoOpenDal*); dalWarmLiveAttachOpenViews_ if those views are already open when the room warms. Soft leave + END ROOM unchanged. Cold short sessions still show Start.
...
END PREP (local or peer)
  ...
  → On Sheets/reconcile fail: **restore previous Sheets**, reopen session, keep fork + backup, set **`dal_commit_retry/{projectId}` needsRetry** (fail-safe C); **push alert ROOT only** (no logistics fan-out; no repeat sweep toasts)
  → Mirror mismatch / fork-cleanup-after-OK: **audit only** (no push — not a Sheets commit failure)
