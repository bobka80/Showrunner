...
- [x] **Station shell split (Phase A, 2026-07-11)** — Monolithic `11_Station_Shell.html` → `11b`–`11l` modules (≤~1k lines each); `stationActiveUiSkin_()` in `11a`. Behavior-neutral on phone sled. Map: [STATION_UI.md](../STATION_UI.md), [FILE_MAP.md](../FILE_MAP.md) §11.
...
## In progress / next (director priority)

1. **[ ] Dock panel UI (Phase B)** — right scan rail, bottom eject, sidebar nav, scale control, screensaver. Scaffold: `11k`, `11l`. Spec: [STATION_UI.md](../STATION_UI.md).
2. **[ ] Kiosk auto-start (APK)** — default launcher + `BOOT_COMPLETED` + battery optimization off.

### Completed priority queue (2026-07-08 — archived here)
