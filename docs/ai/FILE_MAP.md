...
- **`11_Station_Shell.html`**: **Warehouse gun UI** — station login takeover, crew badge host inherit, `window.onStationRfidScan` SDK hook. APIs: `getStationShellBootstrap`, `processStationRfidScan`, `enrollStationCrewRfidTag`. **Always-on top "Live RFID scans" strip** (every EPC, any state; last `STATION_SCAN_FEED_MAX`=8). **Self-serve badge enrollment** — while hosted, "Link my RFID badge" captures the next scan and writes it to the host's `Crew_Roster.rfid_tag`. **Host idle auto-eject** after `STATION_HOST_IDLE_MS` (10 min) — resets on touch/scan, ejects host only (device stays logged in).
