...
- **`station-desktop/`**: **Native desktop/TV station shell** for the **TSL 1128-EU** gun (Windows WPF + **WebView2**, .NET 8). `ShowrunnerStationDesktop/`: `TslRfidManager.cs` (TSL ASCII protocol over Bluetooth COM), `GunPortDetector.cs` (auto-detect reader by `PID_1128`, no COM config), `StationBridge.cs` (exposes `window.AndroidStation` — same API as the Chainway APK so `11_Station_Shell.html` needs no fork), `DesktopPrefs.cs` (`desktop-prefs.json` in `%LocalAppData%`), `MainWindow.xaml.cs`. Drives the `tsl_dock_desktop` layout; app-sleep via ASCII `.sl`. See `station-desktop/README.md` + [active/rfid-station-profiles.md](active/rfid-station-profiles.md) § Desktop TSL station.
