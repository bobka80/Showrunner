...
- **Device settings** (`RfidManager`): read **power/sensitivity** (`setPower`, 5–30 dBm), **scan mode** — `single` / `continuous` (pull to toggle a burst) — **continuous repeat speed** (`setPollMs`, 100–2000 ms — drives the `continuousRunnable` repeat), **beeper** (`setBeep`) — persisted in Android prefs, reapplied on reconnect; battery/firmware read on connect. The web setup is the source of truth and pushes these down on every boot, so the screen and gun never disagree.
- **Web ↔ native bridge:**
  - Scans (primary, build 11+): `evaluateJavascript` can only reach the **top frame**, so the relay path proved lossy. Reads are now **queued natively** (`pendingScans`) and the iframe **pulls** them directly via `AndroidStation.pollScans()` every 300 ms — the same proven direct channel as settings. The relay below stays as a fallback (deduped client-side).
  - Scans (fallback): `deliverEpcToShowrunner` calls `showrunnerStationDeliverScan` in the shell (host-boot.js), which `postMessage`s `SHOWRUNNER_RFID_SCAN` into the iframe (or a direct `onStationRfidScan` when pointed straight at GAS).
  - Scan modes: **Single / Continuous** only. Hold was removed — the R6 delivers **no key-up** on trigger release (`onKeyUp` never fires), so hold-to-scan can't stop on release.
  - Settings: `AndroidStation` `@JavascriptInterface` (`getConfig`/`setPower`/`setScanMode`/`setPollMs`/`setBeep`). Android injects the interface into **every frame**, so the station shell (inside the iframe) calls it **directly** for live apply, falling back to the `SHOWRUNNER_STATION_CONFIG_GET/SET` relay through the hosting shell only when the direct interface isn't present. Gun radio settings can only change via this native layer — the web cannot touch the gun on its own (only the eject timer is pure-web).
  - Diagnostics (build 10, retired v432): verbose `postStatus` trigger/settings/read echoes removed; connection lifecycle messages kept.
  - Startup: a **native kiosk splash** (`activity_station_web.xml` `station_splash`) covers the WebView until the station shell mounts. `host-boot.js` relays `SHOWRUNNER_STATION_READY`→`AndroidStation.shellReady()` (and a login screen→`loginNeeded()`); 30 s safety timeout. The device must be signed into its **station-device** account, or it lands on the normal dashboard.
...
- **App "restarts"/cold-boots every time the gun disconnects or reconnects (fixed APK 0.1.36, build 38):** the gun is a Bluetooth **HID keyboard** (the trigger is a `KeyEvent`), so connecting/disconnecting it changes Android's `keyboard`/`navigation` device configuration. `AndroidManifest.xml` must list **`keyboard` and `navigation`** in `android:configChanges` (alongside `keyboardHidden|orientation|screenSize|…`) or Android **destroys and recreates `StationWebActivity`** on every gun flap — the WebView rebuilds, the station cold-boots (SYSTEM SECURE, lost host badge, ~20s). With them declared, the flap is delivered to `onConfigurationChanged` and absorbed with the WebView intact (breadcrumb: quiet "Gun link changed — session kept" toast via `stationOnGunConfigChange_`). **Never remove `keyboard`/`navigation` from `configChanges`.** This was the true root cause of the long-standing "reboots on gun sleep" bug — not a web/session issue. See [FRAGILE_ZONES.md](../docs/ai/FRAGILE_ZONES.md) § Station native app — Android Activity lifecycle.
