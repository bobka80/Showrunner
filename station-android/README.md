...
- **Web ↔ native bridge:**
  ...
  - Settings: `AndroidStation` `@JavascriptInterface` (`getConfig`/`setPower`/`setScanMode`/`setPollMs`/`setBeep`) is driven from the web setup view via `SHOWRUNNER_STATION_CONFIG_GET/SET` relayed through the hosting shell.
