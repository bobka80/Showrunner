...
- **Web ↔ native bridge:**
  ...
  - Diagnostics (build 10): setters `postStatus("Gun set: …")` and the trigger posts `Trigger DOWN/UP [mode]` to the native status bar; the shell shows an on-screen breadcrumb overlay (`#station-debug`) with a `station=<bool> native=<bool>` badge and per-hop scan/settings logs.
